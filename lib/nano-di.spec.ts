...
describe("Lightweight DI Library", () => {
  ...
  it("should throw an error for unbound dependencies", () => {
    ...
    expect(() => container.resolve<ServiceA>("ServiceA")).toThrow(
      "No binding found for identifier: ServiceA",
    );
  });
  ...
  it("should resolve multiple levels of dependencies", () => {
    ...
    container
      .bind("ServiceA", ServiceA)
      .bind("ServiceB", ServiceB)
      .bind("ServiceC", ServiceC);

    const serviceC = container.resolve<ServiceC>("ServiceC");

    expect(serviceC).toBeInstanceOf(ServiceC);
    expect(serviceC.getCombinedMessage()).toBe(
      "Hello from ServiceA! And ServiceC too!",
    );
  });
